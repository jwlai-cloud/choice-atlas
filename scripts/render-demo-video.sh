...
ffmpeg -y -loop 1 -t 5 -i outputs/choice-atlas-hero.png -vf "scale=1920:1080,drawbox=x=0:y=0:w=1920:h=1080:color=0x1e2a27@0.42:t=fill,drawtext=fontfile=${FONT}:text='CHOICE ATLAS':fontcolor=white:fontsize=78:x=(w-text_w)/2:y=455,drawtext=fontfile=${FONT}:text='A better map for the decision you still own':fontcolor=0xe5d3a0:fontsize=32:x=(w-text_w)/2:y=540,fade=t=in:st=0:d=0.4,fade=t=out:st=4.4:d=0.6" -r 30 -an "$OUT/07-close.mp4"

ffmpeg -y -i "$OUT/01-hero.mp4" -i "$OUT/02-intake.mp4" -i "$OUT/03-live-proof.mp4" -i "$OUT/04-weather.mp4" -i "$OUT/05-architecture.mp4" -i "$OUT/06-proof.mp4" -i "$OUT/07-close.mp4" -i outputs/choice-atlas-voiceover.aiff -filter_complex "[0:v]setsar=1[v0];[1:v]setsar=1[v1];[2:v]setsar=1[v2];[3:v]setsar=1[v3];[4:v]setsar=1[v4];[5:v]setsar=1[v5];[6:v]setsar=1[v6];[v0][v1][v2][v3][v4][v5][v6]concat=n=7:v=1:a=0[v];[7:a]loudnorm=I=-16:LRA=11:TP=-1.5[a]" -map "[v]" -map "[a]" -c:v libx264 -preset medium -crf 19 -pix_fmt yuv420p -c:a aac -b:a 192k -t 120 "$OUT/choice-atlas-build-week-demo-1080p.mp4"
